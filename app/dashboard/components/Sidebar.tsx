...
export default function Sidebar({ className = '' }: SidebarProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { id: 'stock', label: 'Movimientos', icon: FileText, route: '/dashboard/stock' },
  ...
  return (
    <motion.div
      ref={sidebarRef}
      className={`fixed left-0 top-0 h-screen z-10 bg-white/90 backdrop-blur-2xl border-r border-slate-200 shadow-2xl transition-all duration-300 ${
      ...
      <div className="p-6 border-b border-slate-200">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            ...
            >
              <div className="w-10 h-10 bg-gradient-to-r from-purple-600 to-purple-800 rounded-xl flex items-center justify-center shadow-lg">
                <Zap className="w-6 h-6 text-white" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-slate-900">Yei Project</h1>
                <p className="text-xs text-slate-600">Sistema de Inventario</p>
              </div>
            </motion.div>
          )}
            ...
            >
              <Button
                ...
                className={`w-full justify-start gap-3 h-auto p-3 transition-colors ${
                  activeMenu === item.id
                    ? 'bg-purple-100 border border-purple-200 text-slate-900 shadow-lg'
              ...
              >
                ...
                {!isCollapsed && (
                  <>
                    ...
                    {item.badge && (
                      <Badge className="bg-purple-600 text-white text-xs shadow-lg">
                        {item.badge}
                      </Badge>
                    )}
                    {item.submenu && (
            ...
            <AnimatePresence>
              {!isCollapsed && item.submenu && activeSubmenu === item.id && (
                <motion.div
                  ...
                  className="ml-8 mt-2 space-y-1 border-l border-primary/30 pl-4"
                >
